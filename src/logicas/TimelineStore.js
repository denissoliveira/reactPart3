...
export default class LogicaTimeline {
    ...
    comenta(fotoId,textoComentario) {
      ...
      fetch(con+`/fotos/${fotoId}/comment?X-AUTH-TOKEN=${localStorage.getItem('auth-token')}`,requestInfo)
        .then(response => {
          if(response.ok){
    ...
    like(fotoId){
      fetch(con+`/fotos/${fotoId}/like?X-AUTH-TOKEN=${localStorage.getItem('auth-token')}`,{method:'POST'})
        .then(response => {
          if(response.ok) {
            return response.json();
          } else {            
